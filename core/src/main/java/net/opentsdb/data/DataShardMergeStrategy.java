 ...
 * {@link #merge(TimeSeriesQueryId, List, QueryContext, Span)} should throw an 
 * {@link IllegalArgumentException} exception.
 * <p>
 * Note that implementations are not expected to check the {@link TimeSeriesQueryId} 
 * of the individual shards during merge. Instead the ID of the resulting shard
 * is passed in to the {@link #merge(TimeSeriesQueryId, List, QueryContext, Span)} 
 * method. 
 * 
...
public interface DataShardMergeStrategy<T extends TimeSeriesDataType> {
   ...
   * in the list.
   */
  public TimeSeriesIterator<T> merge(final TimeSeriesQueryId id, 
