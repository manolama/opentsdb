...
public class TestGuavaLRUCache {
  ...
  private TimeSeriesCacheSerdesFactory factory;
  ...
  public void before() throws Exception {
    ...
    factory = mock(TimeSeriesCacheSerdesFactory.class);
    serdes = mock(TimeSeriesCacheSerdes.class);
    
    when(tsdb.registry.getPlugin(eq(TimeSeriesCacheSerdesFactory.class), anyString()))
    .thenReturn(factory);
    ...
    when(serdes.deserialize(any(byte[].class))).thenAnswer(
        new Answer<Map<String, CachedQueryResult>>() {
          @Override
          public Map<String, CachedQueryResult> answer(
  ...
  public void cacheAndFetchSingle() throws Exception {
    ...
    Deferred<Void> deferred = cache.cache(BASE_TIME, key1, 60000, qr1);
    ...
    deferred = cache.cache(BASE_TIME, key2, 60000, qr2);
    ...
    deferred = cache.cache(BASE_TIME, key1, 60000, qr3);
    assertEquals(2, cache.cache().size());
    assertEquals(8, cache.bytesStored());
    ...
    deferred = cache.cache(BASE_TIME, key3, -1, qr4);
    assertEquals(2, cache.cache().size());
    assertEquals(8, cache.bytesStored());
    assertEquals(3, serdes_calls.size());
    assertNull(deferred.join());
    ...
    // fetch
    CacheQueryResults[] results = new CacheQueryResults[2];
    Throwable[] errors = new Throwable[2];
    class CB implements CacheCB {
      @Override
  ...
  public void cacheAndFetchSingleExceptions() throws Exception {
    ...
    Deferred<Void> deferred = cache.cache(BASE_TIME, null, 60000, qr1);
    try {
      deferred.join();
      fail("Expected IllegalArgumentException");
    } catch (IllegalArgumentException e) { }
    
    deferred = cache.cache(BASE_TIME, new byte[0], 60000, qr1);
    try {
      deferred.join();
      fail("Expected IllegalArgumentException");
    } catch (IllegalArgumentException e) { }
    ...
    deferred = cache.cache(BASE_TIME, key1, 60000, qr1);
    assertEquals(1, cache.cache().size());
    assertEquals(4, cache.bytesStored());
    assertEquals(1, serdes_calls.size());
    assertNull(deferred.join());
  ...
  static class SerdesObj {
    ...
    final Map<String, CachedQueryResult> deserialized;
