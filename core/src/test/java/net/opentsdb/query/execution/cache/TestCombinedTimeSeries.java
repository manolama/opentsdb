...
public class TestCombinedTimeSeries {
  ...
  public void ctorAndAddAndClose() throws Exception {
    CombinedResult result = mockResult(3);
    TimeSeries ts1 = mockSeries(NumericType.TYPE);
    CombinedTimeSeries cts = new CombinedTimeSeries(result, 0, ts1);
  ...
  public void iterator() throws Exception {
    ...
    assertTrue(it.get() instanceof CombinedNumeric);
    ...
    cts = new CombinedTimeSeries(result, 0, ts1);
    cts.add(1, ts2);
    
    it = cts.iterator(NumericArrayType.TYPE);
    assertTrue(it.isPresent());
    assertTrue(it.get() instanceof CombinedArray);
    ...
    assertTrue(it.isPresent());
    assertTrue(it.get() instanceof CombinedSummary);
  }
  ...
  public void iterators() throws Exception {
    ...
    assertTrue(iterators.iterator().next() instanceof CombinedNumeric);
    ...
    assertEquals(1, iterators.size());
    assertTrue(iterators.iterator().next() instanceof CombinedArray);
    ...
    iterators = cts.iterators();
    assertEquals(1, iterators.size());
    assertTrue(iterators.iterator().next() instanceof CombinedSummary);
  }
  
  CombinedResult mockResult(final int num_results) {
    ...
    CombinedResult result = new CombinedResult(context, results, node, 
        data_source, sinks, latch, "1h");
    return result;
  }
