...
//      called_prediction_cache = new AtomicBoolean[1];
//      called_prediction_cache[0] = new AtomicBoolean();
  ...
  void runBaseline(final int prediction_idx) {
    ...
    LOG.info("[EGADS] Running baseline with properties: " + properties.toString() + "  For index: " + prediction_idx);
    ...
    while (it.hasNext()) {
      ...
      LOG.info("[EGADS] Baseline series: " + ts.id() + "  For index: " + prediction_idx);
      if (ts != null) {
        computed.add(ts);
      }
    }
...
//      if (called_prediction_cache[prediction_index].compareAndSet(false, true)) {
//        for (int i = 0; i < predictions.length; i++) {
//          cache.fetch(pipelineContext(), cache_keys[prediction_index], null)
//            .addCallback(new CacheCB(i))
//            .addErrback(new CacheErrCB(i));
...
//            + "fetch more than once for " + prediction_index);
//      }
      return true;
