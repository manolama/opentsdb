...
public class RedisClusterPredictionCache extends BaseTSDBPlugin 
  ...
  public Deferred<Object> initialize(final TSDB tsdb, final String id) {
    ...
    final String shared_object = tsdb.getConfig().getString(RedisClusterQueryCache.SHARED_OBJECT_KEY);
    ...
    final String serdes_id = tsdb.getConfig().getString(RedisClusterQueryCache.SERDES_KEY);
  ...
  public Deferred<Void> cache(final byte[] key, 
    ...
    try {
      ...
      cluster.set(key, data, RedisClusterQueryCache.NX, RedisClusterQueryCache.EXP, expiration);
  ...
  public void setState(final byte[] key, 
    ...
    try {
      cluster.set(prefixed_key, data, RedisClusterQueryCache.NX, RedisClusterQueryCache.EXP, expiration);
      tsdb.getStatsCollector().incrementCounter("query.cache.redis.set", 
          (String[]) null);
    } catch (Exception e) {
      LOG.error("Unexpected exception writing to Redis.", e);
