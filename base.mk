...
$(tsdb_SRC): $(dependencies)
	@$(refresh_src)

find_jar = test -f "$$jar" && echo "$$jar" || echo "$(srcdir)/$$jar"
get_dep_classpath = `for jar in $(dependencies); do $(find_jar); done | tr '\n' ':'`
...
install-data-lib: $(dependencies) $(jar)
	@$(NORMAL_INSTALL)
	@list='$(dependencies) $(jar)'; test -n "$(pkgdatadir)" || list=; \
...
install-data-tools: $(dependencies) $(jar)
	@$(NORMAL_INSTALL)
...
get_runtime_dep_classpath = `for jar in $(test_dependencies); do $(find_jar); done | tr '\n' ':'`
$(test_SRC): $(test_dependencies)
	@$(refresh_src)
	
$(test_plugin_SRC): $(test_dependencies)
	@$(refresh_src)
...
debian: dist staticroot
	...
	`for dep_jar in $(dependencies); do cp $$dep_jar \
...
.PHONY: all jar doc check gwtc gwtdev printdeps staticroot gwttsd rpm
include third_party/include.mk

printdeps:
	@echo $(dependencies) $(jar)