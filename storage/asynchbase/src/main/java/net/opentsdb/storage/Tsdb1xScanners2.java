 ...
 * @since 3.0
 */
public class Tsdb1xScanners2 implements HBaseExecutor {
  ...
  public void close() {
    if (scanners != null) {
      for (final Tsdb1xScanner2[] scnrs : scanners) {
        for (final Tsdb1xScanner2 scanner : scnrs) {
          try {
            scanner.close();
          } catch (Exception e) {
  ...
  public State state() {
    ...
    for (final Tsdb1xScanner2 scanner : scanners.get(scanner_index)) {
    ...
    try {
      ...
      if (node.rollupIntervals() != null && 
        ...
        for (int i = 0; i < node.rollupIntervals().size(); i++) {
          ...
          for (int x = 0; x < array.length; x++) {
            ...
            array[x] = new Tsdb1xScanner2(Tsdb1xScanners2.this, scanner, x, interval);
          }
          idx++;
      ...
      if (node.rollupIntervals() == null || 
        ...
        for (int i = 0; i < array.length; i++) {
          ...
          array[i] = new Tsdb1xScanner2(this, scanner, i, null);
        }
      }
      initialized = true;
    } catch (Exception e) {
  ...
  void scanNext(final Span span) {
    ...
    final Tsdb1xScanner2[] scnrs = scanners.get(scanner_index);
    for (final Tsdb1xScanner2 scanner : scnrs) {
